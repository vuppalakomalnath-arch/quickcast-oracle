...
import { Wallet, Menu, X, CircleDot, Briefcase, ChevronDown, LogOut, Copy, ExternalLink } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useWallet } from "@/context/WalletContext";
import { toast } from "sonner";

const Navbar = () => {
  const { connected, address, balance, network, portfolioValue, openPositions, connect, disconnect } = useWallet();
            ...
            <AnimatePresence>
              {walletDropdown && connected && (
                <motion.div
                  ...
                  className="absolute right-0 top-full mt-2 w-72 glass-card p-4 space-y-3 z-50"
                >
                  ...
                  <div className="border-t border-border pt-3 space-y-2">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-semibold text-foreground">{balance.toLocaleString()} ALGO</span>
                    </div>
                    <div className="flex justify-between text-xs">
